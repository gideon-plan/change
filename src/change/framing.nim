## framing.nim -- Tier envelope for change messages.
##
## Each change message is wrapped in a tier envelope that indicates
...
proc encode_frame*(frame: ChangeFrame): seq[byte] =
  ## Encode a change frame to bytes: [tier:1][kind:1][seq_no:8][len:4][payload]
...
proc decode_frame*(data: openArray[byte]): Choice[ChangeFrame] =
  ## Decode a change frame from bytes.
  if data.len < 14:
    return bad[ChangeFrame]("change", "frame too short: " & $data.len)
  let tier = ChangeTier(data[0])
  let kind = ChangeFrameKind(data[1])
  var seq_no: uint64 = 0
  for i in 0 ..< 8:
  ...
  if data.len < 14 + int(plen):
    return bad[ChangeFrame]("change", "frame payload truncated")
  var payload = newSeq[byte](plen)
  for i in 0 ..< int(plen):
    payload[i] = data[14 + i]
  good(ChangeFrame(tier: tier, kind: kind, seq_no: seq_no, payload: payload))
